...
const KNOWN_META_COLS = new Set([
  ...
  'hindi', 'rubric_hindi', 'subrubric_hindi', 'medicine'
  // NOTE: 'grade' and 'grading' intentionally EXCLUDED — they must be detected as grade columns
]);
...
const parseExcel = async (buffer) => {
  ...
  for (const sheetName of workbook.SheetNames) {
    ...
    // Filter out rows that are EXACT duplicates of header rows only.
    // IMPORTANT: We use EXACT cell-value matching against a fixed header set.
    // Partial/substring matching (the old approach) was incorrectly deleting
    // Kent data rows whose cells contain words like 'chapter', 'rubric', 'grading'.
    const EXACT_HEADER_WORDS = new Set([
      'chapter (english)', 'chapter (hindi)', 'rubric (english)', 'rubric (hindi)',
      'medicine', 'grading', 'grade', 'subrubric', 'sub-rubric', 'sub rubric',
      'chapter', 'rubric', 'section', 'aggravation', 'amelioration', 'synonyms'
    ]);
    rawRows = rawRows.filter(row => {
      const rowValues = Object.values(row).map(v => String(v).toLowerCase().trim()).filter(Boolean);
      if (rowValues.length === 0) return false; // skip blank rows
      // A row is a header row ONLY if ALL its non-empty cells are exact known header words
      const allAreHeaders = rowValues.every(val => EXACT_HEADER_WORDS.has(val));
      if (allAreHeaders && rowValues.length >= 2) {
        console.log(`⏭️  Skipping duplicate header row:`, rowValues.slice(0, 4).join(', '));
        return false;
      }
      return true;
    });
    ...
    if (rowBasedCols) {
      ...
      // ── KENT RUBRIC PATH SPLITTING ──────────────────────────────────────────────
      // The Rubric (English) column stores the FULL hierarchical path generated by
      // kentExcelGenerator.js, e.g.:
      //   "FOREIGN body"                              → rubric only, no subrubric
      //   "FOREIGN body - smoking, while"             → rubric + subrubric
      //   "FOREIGN body - smoking, while - not amel." → rubric + sub + sub-sub
      //
      // Kent typography: MAIN RUBRIC = first segment (ALL-CAPS or starts uppercase).
      // Subrubric = everything after the first " - " separator (lowercase text).
      //
      // We also strip any leading chapter name prefix if the AI included it,
      // e.g. "THROAT - FOREIGN body - smoking" → strips "THROAT - " prefix.
      // ────────────────────────────────────────────────────────────────────────────
      const splitKentRubricPath = (fullPath, chapterName) => {
        if (!fullPath) return { rubric: '', subrubric: '' };
        let path = fullPath.trim();

        // Strip leading chapter prefix (e.g. "THROAT - " or "THROAT-") if present
        if (chapterName) {
          const chapUpper = chapterName.toUpperCase().trim();
          const chapRegex = new RegExp(
            `^${chapUpper.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*-\\s*`, 'i'
          );
          path = path.replace(chapRegex, '').trim();
        }

        // Split on " - " (the Kent separator used by kentExcelGenerator)
        const segments = path.split(/\s+-\s+/);
        const rubric    = segments[0].trim();
        const subrubric = segments.slice(1).join(' - ').trim(); // rejoin sub-parts
        return { rubric, subrubric };
      };

      const splitKentHindiPath = (fullPath, chapterHi) => {
        if (!fullPath) return { rubric: '', subrubric: '' };
        let path = fullPath.trim();
        if (chapterHi) {
          const chapRegex = new RegExp(
            `^${chapterHi.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*-\\s*`
          );
          path = path.replace(chapRegex, '').trim();
        }
        const segments = path.split(/\s+-\s+/);
        const rubric    = segments[0].trim();
        const subrubric = segments.slice(1).join(' - ').trim();
        return { rubric, subrubric };
      };
      ...
      rawRows.forEach((row, idx) => {
        ...
        if (effectiveChapter) lastChapter = effectiveChapter;
        const effectiveChapterHi = fields.chapterHi || lastChapterHi;
        if (fields.chapterHi) lastChapterHi = fields.chapterHi;

        const currentChapterEn = effectiveChapter || lastChapter;
        const currentChapterHi = effectiveChapterHi;

        // Split English rubric path into main rubric + subrubric
        const { rubric: splitRubricEn, subrubric: splitSubrubricEn } =
          splitKentRubricPath(fields.rubricEn, currentChapterEn);

        // Split Hindi rubric path into main rubric + subrubric
        const { rubric: splitRubricHi, subrubric: splitSubrubricHi } =
          splitKentHindiPath(fields.rubricHi, currentChapterHi);

        // Forward-fill rubric context
        let effectiveRubric   = splitRubricEn   || lastRubric;
        let effectiveRubricHi = splitRubricHi   || lastRubricHi;
        if (splitRubricEn) {
          lastRubric   = splitRubricEn;
          lastRubricHi = splitRubricHi || '';
        }

        // Forward-fill subrubric context
        let effectiveSubrubric   = splitSubrubricEn;
        let effectiveSubrubricHi = splitSubrubricHi;

        if (splitRubricEn) {
          // New main rubric — take its own subrubric (may be empty)
          lastSubrubric   = splitSubrubricEn;
          lastSubrubricHi = splitSubrubricHi;
        } else if (splitSubrubricEn) {
          // Same main rubric, new subrubric line
          lastSubrubric   = splitSubrubricEn;
          lastSubrubricHi = splitSubrubricHi;
        } else {
          // Carry forward subrubric from previous row
          effectiveSubrubric   = lastSubrubric;
          effectiveSubrubricHi = lastSubrubricHi;
        }

        // Skip row if no chapter or rubric is present
        if (!currentChapterEn && !effectiveRubric) {
          return;
        }
        ...
        const parts = [
          currentChapterEn, currentChapterHi,
        ...
        if (existingRubric) {
          ...
          rubrics.push({
            chapter:   { en: currentChapterEn || 'General', hi: currentChapterHi },
